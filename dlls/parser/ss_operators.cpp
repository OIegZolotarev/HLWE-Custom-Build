...
Type Of::execute()
{
	...
	return s.vectv.v[offset];//To support global structures, we don't checking type. In different scripts same structure types can have different ids
}
...
/*
	...
	Old bullshit for school dulls. Now do all job with my own hands:*/

	for (unsigned int i = 0; i < operators->size(); ++i)
	{
		Type retval = operators->at(i)->execute();

		if (retval.my_type == _BREAK || retval.my_type == _CONTINUE || retval.my_type == _RETURN)
		{
			_s->Clear();
...
Type If::execute()
{
	...
	else if ( has_bottom )
	{
		...
		*/
		...
		retval = bottom->execute();
	}

	_s->Clear();

	return retval;//_ZERO will be returned by default. In unusual case - _CONTINUE or something...
}
		...
		if ( has_body )
		{
			...
			*/
			...
			else if (retval.my_type == _BREAK)
			{
				...
				{
					break;//exit cycle, return _zero
				}
			}
			else if (retval.my_type == _RETURN)
			{
				_s->Clear();

				return retval;//exit cycle, return value to superior handler
			}
		}
	}
...
Type For::execute()
{
	...
	if ( blocks & CHECK )
	{
		...
		{
			_s->Clear();//Did I forgot this in previous version???
		...
		*/
		...
		else if (retval.my_type == _BREAK)
		{
			...
			else
			{
				...
				return _ZERO;//Just finish cycle, return _zero
			}
		}
		else if (retval.my_type == _RETURN)
		{
