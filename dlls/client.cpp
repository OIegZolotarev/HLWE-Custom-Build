...
//LLAPb begin
#include "ss_mgr.h"
CScriptMgr *pScriptMgr = 0;
extern int errors;

Script* CreateScript ( char *filename )
{
	if (!pScriptMgr) pScriptMgr = new CScriptMgr;

	return pScriptMgr->PasteScript(filename);
}

void FreeScript ( Script *scr )
{
	if (pScriptMgr)
		pScriptMgr->UnlinkScript(scr);
}
...
{       
	if (!FStrEq(STRING(pev->model), "models/player/gordon/gordon.mdl"))
...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq(pcmd, "drop" ) )
	{
		...
		if (!GetClassPtr((CBasePlayer *)pev)->IsObserver())
   ...
   //END BOT
	...
	else if (FStrEq(pcmd, "cloaktoggle" )) 
	{ 
	if (pPlayer->m_fCloak)
		{
			...
			pPlayer->CloakToggle(TRUE);
		}
	}
	else if (FStrEq(pcmd, "antigrav" )) 
	{ 
		if (pPlayer->m_fAntigrav)
		{
			...
			pPlayer->AntigravToggle(TRUE);
		}
	}
	else if (FStrEq( pcmd, "switch" ))
	{
		if (pPlayer->m_pActiveItem)
	{
		CBasePlayerItem *pGun = pPlayer->m_pActiveItem->GetWeaponPtr();
		if (pGun)
			((CBasePlayerWeapon*)pGun)->SwitchFireMode();
		}
	}
...
//================
	...
	{
		g_pGameRules->ShowMenu (pPlayer);
	}
	else if (!strncmp(pcmd, "buy_", 4))
	{
		g_pGameRules->InterpretBuyCommand (pcmd, pPlayer);
	}

	else if ( FStrEq(pcmd, "end_spectate" ) )
	{
		if (g_pGameRules->IsLMS()) return;

		if (g_pGameRules->IsCTF() && pPlayer->m_iClass == 0) return;

		if (gpGlobals->time < pPlayer->m_fDeadTime + mp_deadtime.value) return;
...
void ServerActivate( edict_t *pEdictList, int edictCount, int clientMax )
{
	...
	//LLAPb begin
	if (!g_serveractive && !pScriptMgr)
	{
		pScriptMgr = new CScriptMgr;
	}
	//LLAPb end
...
void StartFrame( void )
{
   ...
   //OutputDebugString("g_pGameRules->Think()\n");
	...
	gpGlobals->teamplay = mp_teamplay.value;
	if (mp_deadtime.value < 1.0)
		mp_deadtime.value = 1.0;
...
const char *GetGameDescription()
{
	if ( g_pGameRules ) // this function may be called before the world has spawned, and the game rules initialized
		...
		return "Half-Life 1.5:Weapon Edition";
}
...
//Observer code
	...
	if ( (pPlayer->pev->iuser2 != 0) && (pPlayer->pev->iuser2 != ENTINDEX(pPlayer->edict())) && (pPlayer->m_hObserverTarget != NULL) )
	{
