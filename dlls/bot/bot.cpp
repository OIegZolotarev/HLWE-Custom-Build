...
char *bot_names[MAX_SKINS] = 
{
	...
	"Cherepnoy_Xrust", 
	"}|{nets_Dush", 
	"Sopliviy_Pjanitsa_[MOLOT]",
	"Trupniy_Ogon'", 
	"Poxititel'_Krovi", 
	"Groznoe_Prikosnovenie", 
	"Shaxtnoe_Gnienie", 
	"Sopliviy_ROG",
	"Y}|{asnie_Kleshi_[TEN']",
	"Grozniy_Utroba_[GOLODNIY]",
	"Mrachnoe_Serdce_[NARKOMAN]",
	"Dushevniy_O}|{OG_[DUX]",
	"Rvotnoe_Krilo_[NECHIST']",
	"Nochnoy_Edok",
	"Izvrachenniy_Polzun",
	"Ognenniy_Oskal",
	"Zaplesnevelaja_Vdova",
	"Rvotniy_Edok",
	"Doxliy_Virodok",
	"Mrachniy_Trup_[**]=======>"
};
...
void BotCreate(const char *skin, const char *name, const char *skill)
{
   ...
   {
      // pick a random skin
      index = RANDOM_LONG(0, 21);  // there are 22 possible skins
      ...
      if (stat(filename, &stat_str) != 0)
      {
         ...
         if (stat(filename, &stat_str) != 0)
         {
            ...
            UTIL_ClientPrintAll( HUD_PRINTNOTIFY,
               "use bot, cloak, csarab, csleet, engineer, fox,\n");
            if (IS_DEDICATED_SERVER())
               printf("use bot, cloak, csarab, csleet, engineer, fox,\n");
            UTIL_ClientPrintAll( HUD_PRINTNOTIFY,
               " gibman, gordon, grunttower, hans, hgrunt or ivan\n");
            if (IS_DEDICATED_SERVER())
               printf(" gibman, gordon, grunttower, hans, hgrunt or ivan\n");
            return;
         }
      }
...
void CBot::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
   if (g_pGameRules->IsTeamplay())  // is team play enabled?
   {
                          ...
                          g_pGameRules->GetTeamID(pActivator)))
      {
         if (pBotEnemy == NULL)  // is bot NOT currently engaged in combat?
         {
            if (pBotUser == NULL)  // does bot NOT have a "user"
            {
              ...
              EMIT_SOUND(ENT(pActivator->pev), CHAN_VOICE, "barney/teamup2.wav", 1.0, ATTN_NORM);
            ...
            else
            {
               ...
               EMIT_SOUND(ENT(pActivator->pev), CHAN_VOICE, "barney/seeya.wav", 1.0, ATTN_NORM);
         ...
         {
            EMIT_SOUND(ENT(pActivator->pev), CHAN_VOICE, "barney/ba_raincheck.wav", 1.0, ATTN_NORM);
         }
      }
   }
}
         ...
         else
         {
            ...
            if (strcmp(item_name, STRING(tr.pHit->v.classname)) == 0)
            {
               ...
               else if (strcmp("func_recharge", item_name) == 0)
               {
                  // check if the bot can use this item and
                  // check if the recharger is ready to use (has power left)...
                  if ((pev->armorvalue < MAX_NORMAL_BATTERY) && (pev->weapons & (1<<WEAPON_SUIT)) && (pEntity->pev->frame == 0))
                  {
		...
		if (pEntity->pev->effects & EF_NODRAW)
            ...
            else if (strcmp("monster_tripmine", item_name) == 0)
            {
               ...
               else
               {
                  ...
                  // see if bot is far enough to shoot the tripmine...
                  if (distance >= 500)  // 500 is damage radius
                  {
                     b_shoot_tripmine = TRUE;
                  }
               }
