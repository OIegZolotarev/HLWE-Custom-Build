...
void UTIL_BloodDecalTrace( TraceResult *pTrace)
{
...
void UTIL_BubbleTrail(Vector vecSrc, Vector vecEnd, BOOL Splash)
{
//moved to client!
	float flHeight = UTIL_WaterLevel(vecSrc, vecSrc.z,vecSrc.z+256);
	flHeight = flHeight-vecSrc.z;

	if (flHeight < 8)
	{
		flHeight = UTIL_WaterLevel(vecEnd,vecEnd.z,vecEnd.z+256);
		flHeight = flHeight-vecEnd.z;
		if (flHeight < 8)
			return;

		flHeight = flHeight + vecEnd.z - vecSrc.z;
	}
}
...
void UTIL_SetModel( edict_t *e, const char *model )
{
	if (!model || !*model) //model not specified ???
	{
		...
		SET_MODEL( e, "models/explosions.mdl");
		return;
	}
...
int UTIL_PrecacheModel( char* s )
{
	...
	{
          	FREE_FILE( (char *)pFile );
		model = PRECACHE_MODEL( "models/explosions.mdl" );
	}
	return model;
...
int UTIL_PrecacheAurora( const char *s )
{
	...
	else //otherwise
	{
		if(!s || !*s)
			...
			ALERT( at_console, "Warning: Aurora %s not found!\n", s );
		return ALLOC_STRING( "aurora/error.aur");
	} 
}

void UTIL_SetAurora( CBaseEntity *pAttach, int aur, int attachment )
{
	MESSAGE_BEGIN( MSG_ALL, gmsgAurora );
