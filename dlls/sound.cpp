...
char TEXTURETYPE_Trace(TraceResult *ptr,  Vector vecSrc, Vector vecEnd)
{
	...
	if (pEntity && (pEntity->pev->takedamage == DAMAGE_AIM))
	{
		...
		else if (pEntity->IsPlayer())
		{
			...
			if (pPlayer->pev->armorvalue)
				chTextureType = CHAR_TEX_ENERGYSHIELD;
			else if (pPlayer->m_fHeavyArmor) 
				chTextureType = CHAR_TEX_BP_METAL;
//			else if (pPlayer->m_fShieldProtection)
//				chTextureType = CHAR_TEX_BP_METAL;
		}
	}
	else if (pEntity && FClassnameIs(pEntity->pev, "func_breakable") || FClassnameIs(pEntity->pev, "func_pushable"))
	{
		...
		if (pBreakable)
		{
			...
			case matBrick:
				chTextureType = CHAR_TEX_BRICK; break;
			case matIce:
				chTextureType = CHAR_TEX_SNOWROCK; break;
			case matSandwall:
				chTextureType = CHAR_TEX_SANDWALL; break;
			}
		}
	}
	else
...
char SURFACETYPE_Trace(TraceResult *ptr, Vector vecSrc, Vector vecEnd)
{
	...
	if (FClassnameIs(pEntity->pev, "func_pushable") || FClassnameIs(pEntity->pev, "func_breakable") || FClassnameIs(pEntity->pev, "func_door"))
	{
		chSurfaceType = SURFACE_BREAKABLE;
	}
	else if (pEntity && (pEntity->pev->takedamage == DAMAGE_AIM))
	{
		if (pEntity->Classify() == CLASS_MACHINE)
		{
			if (pEntity->pev->armorvalue)
		...
		else if (pEntity->IsPlayer())
		{
			...
			if (pPlayer->pev->armorvalue)
				...
				chSurfaceType = SURFACE_NONE;//пока мы не можем определить, попала ли пуля в щит или в тело, поэтому рисуем эффект поверхности в коде игрока
			else
				chSurfaceType = SURFACE_FLESH;
		}
		else if (pEntity->Classify() == CLASS_DEAD)
			chSurfaceType = SURFACE_FLESH;
	}
