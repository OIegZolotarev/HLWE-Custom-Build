...
//
void CopyToBodyQue(entvars_t *pev, BOOL Ignite) 
{
	...
	pevHead->angles.x = pevHead->angles.z = 0;
	UTIL_MakeVectors ( pevHead->angles );

	Vector right, forward, up = Vector( 0, 0, 20 );
	TraceResult tr1, tr2;
	float angle_z;

	UTIL_TraceLine ( pevHead->origin + up, pevHead->origin - up, ignore_monsters, ENT(pevHead), &tr1 );
	UTIL_TraceLine ( pevHead->origin + gpGlobals->v_right * 5 + up, pevHead->origin - up + gpGlobals->v_right * 5, ignore_monsters, ENT(pevHead), &tr2 );

	if ( tr1.flFraction < 1.0 && tr2.flFraction < 1.0 )
	{
		up = tr1.vecPlaneNormal;
		right = ( tr2.vecEndPos - pevHead->origin ).Normalize ();
		...
		pevHead->angles = UTIL_VecToAngles ( forward );
		pevHead->angles.z = angle_z * sgn;
	}
	...
	if (Ignite)
	{
		...
		WRITE_COORD( pevHead->origin.x );
		WRITE_COORD( pevHead->origin.y );
		WRITE_COORD( pevHead->origin.z );
		WRITE_SHORT( ENTINDEX(ENT(pevHead)) );
		WRITE_BYTE( PROJ_FLAME_DETONATE );
		MESSAGE_END();
	}
}
...
#define SF_WORLD_TITLE		0x0001		// Display game title at startup
#define SF_NO_BUY		0x0002		// Disable buy system on this map
...
void CWorld :: Precache( void )
{
	...
	CVAR_SET_STRING("sv_maxvelocity", "1000000");
	CVAR_SET_STRING("tracerspeed", "50000");
...
// make sure the .NOD file is newer than the .BSP file.
	...
	if ( pev->spawnflags & SF_NO_BUY )
		CVAR_SET_FLOAT( "mp_nobuy", 1.0 );
	else
		CVAR_SET_FLOAT( "mp_nobuy", 0.0 );
...
void CWorld :: KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "nobuy"))
	{
		pev->spawnflags |= SF_NO_BUY;
		pkvd->fHandled = TRUE;
	}
	else
		CBaseEntity::KeyValue( pkvd );
