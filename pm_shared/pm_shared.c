...
#define PLAYER_LONGJUMP_SPEED 400 // how fast we longjump
...
void PM_InitTextureTypes()
{
	...
	fileSize = pmove->COM_FileSize( "maps/materials/materials.txt" );
	pMemFile = pmove->COM_LoadFile( "maps/materials/materials.txt", 5, NULL );
	if ( !pMemFile )
		return;

	filePos = 0;
	// for each line in the file...
	while ( pmove->memfgets( pMemFile, fileSize, &filePos, buffer, 1023 ) != NULL && (gcTextures < CTEXTURESMAX) )
	{
		// skip whitespace
		i = 0;
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	if ( pmove->multiplayer && ( !g_onladder && Length( hvel ) <= 100 ) )
		return;


	HeavyArmor = atoi( pmove->PM_Info_ValueForKey( pmove->physinfo, "harm" ) ) == 1 ? true : false;
	...
	if (HeavyArmor)
	{
		...
		{
		// right foot
		case 0:	pmove->PM_PlaySound( CHAN_STATIC, "player/pl_step_harmor1.wav", 0.3, ATTN_NORM, 0, PITCH_NORM );break;
		case 1:	pmove->PM_PlaySound( CHAN_STATIC, "player/pl_step_harmor2.wav", 0.3, ATTN_NORM, 0, PITCH_NORM );break;
		// left foot
		case 2:	pmove->PM_PlaySound( CHAN_STATIC, "player/pl_step_harmor2.wav", 0.3, ATTN_NORM, 0, PITCH_NORM );break;
		case 3:	pmove->PM_PlaySound( CHAN_STATIC, "player/pl_step_harmor1.wav", 0.3, ATTN_NORM, 0, PITCH_NORM );break;
		}
	}
	else
	switch (step)
	...
	case STEP_SAND:
		...
		{
		// right foot
		case 0:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_sand1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_sand3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_sand2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_sand4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_ENERGYSHIELD:
		switch(irand)
		{
		// right foot
		case 0:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_shield_impact1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	pmove->PM_PlaySound( CHAN_BODY, "buttons/spark2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_shield_impact3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	pmove->PM_PlaySound( CHAN_BODY, "buttons/spark4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	}
}	
...
int PM_MapTextureTypeStepType(char chTextureType)
{
	switch (chTextureType)
	{
		...
		case CHAR_TEX_BP_CONCRETE:	return STEP_CONCRETE;	
		case CHAR_TEX_METAL: return STEP_METAL;	
		case CHAR_TEX_BP_METAL: return STEP_METAL;	
		...
		case CHAR_TEX_LEAVES: return STEP_GRASS;	
		case CHAR_TEX_SAND: return STEP_SAND;	
		case CHAR_TEX_ENERGYSHIELD: return STEP_ENERGYSHIELD;	
	}
}
...
void PM_Jump (void)
{
	...
	{	// swimming, not jumping
		...
		{
			// Don't play sound again for 0.5 second
			pmove->flSwimTime = 500;
	...
	// Acclerate upward
		if ( cansuperjump && Length( pmove->velocity ) > 125 )
		{
