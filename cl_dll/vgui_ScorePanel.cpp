...
ctf_info_t			 g_ExtraTeamInfo[MAX_TEAMS+1];
...
void ScorePanel::Initialize( void )
{
	...
	memset( g_ExtraTeamInfo, 0, (MAX_TEAMS+1) * sizeof(ctf_info_t) );
}
...
void ScorePanel::SortTeams()
{
	...
	{
		//if ( !g_TeamInfo[i].scores_overriden && gHUD.m_Teamplay < 2 )
			g_TeamInfo[i].frags = g_TeamInfo[i].deaths = 0;

		g_TeamInfo[i].ping = g_TeamInfo[i].packetloss = 0;
	}

	//LLAPb: I am an Idiot, la-la-la *crazy*
	if (gHUD.m_Teamplay >= 2)
	{
		for ( int i = 0; i <= MAX_TEAMS; i++ )
		{
			...
			for (x = 0; x <= MAX_TEAMS; x++)
				if (!strncmp( g_TeamInfo[i].name, g_ExtraTeamInfo[x].name, MAX_TEAM_NAME))
					g_TeamInfo[i].frags = g_ExtraTeamInfo[x].score;
