...
void V_CalcNormalRefdef ( struct ref_params_s *pparams )
{
	...
	// Start of v_model rendering by Ghoul
...
void V_GetChasePos(int target, float * cl_angles, float * origin, float * angles)
{
	...
	bool calcdist = plr->index == g_iUser2;
	...
	else
	{
		...
		if (!calcdist) V_GetChaseOrigin( angles, origin, cl_chasedist->value, origin );
	}
...
int V_FindViewModelByWeaponModel(int weaponindex)
{
	...
	static char * modelmap[][2] =	{NULL, NULL};
...
void DLLEXPORT V_CalcRefdef( struct ref_params_s *pparams )
{
	if (pparams->nextView == 0)// new queue
	{
...
void V_CalcRefdefWorld(ref_params_t *pparams)
{
	...
	{
		if (CL_IsThirdPerson())// includes 'death cam'
