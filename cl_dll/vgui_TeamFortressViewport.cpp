...
void TeamFortressViewport::UpdateSpectatorPanel()
{
	...
	if ( g_iUser1 && gHUD.m_pCvarDraw->value && !gHUD.m_iIntermission)	// don't draw in dev_overview mode
	{
		...
		{	// restore GUI color
			m_pSpectatorPanel->m_BottomMainLabel->setFgColor( 255, 170, 0, 35 );
		}
		...
		int timer = (int)( gHUD.m_Timer.Time - gHUD.m_flTime );
...
CMenuPanel* TeamFortressViewport::CreateTextWindow( int iTextToShow )
{
	...
	CMenuPanel *pMOTDPanel = CMessageWindowPanel_Create( cText, cTitle, g_iPlayerClass == PC_UNDEFINED, false, XRES(55), YRES(55), ScreenWidth - XRES(110), ScreenHeight - YRES(110) );
...
//================================================================
// VGUI Menus
void TeamFortressViewport::ShowVGUIMenu( int iMenu, int PC, int PK )
{
	...
	switch ( iMenu )
	{
	//HLWE Buy menu; Steam designed etc.
	case MENU_WEAPONS:
		{
		if (m_pJoinMenu->isVisible()) break;
		if (m_pBuyMenu->isVisible()) break;
		pNewMenu = ShowBuyMenu();
		break;
		}

	case MENU_AMMO://if player is alive, he can only buy ammo or sell current weapon
		{
		if (m_pJoinMenu->isVisible()) break;
		if (m_pBuyMenu->isVisible()) break;
		pNewMenu = ShowSellMenu();
		break;
		}
...
CMenuPanel* TeamFortressViewport::ShowBuyMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;

	if (!m_pCurrentMenu) m_pBuyMenu->setVisible( true );
	m_pBuyMenu->UpdateState(TYPE_DEFAULT);
...
CMenuPanel* TeamFortressViewport::ShowSellMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	if (!m_pCurrentMenu) m_pBuyMenu->setVisible( true );
	m_pBuyMenu->UpdateState(TYPE_ONLYAMMO);
...
CMenuPanel* TeamFortressViewport::ShowFullMenu()
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	if (!m_pCurrentMenu) m_pBuyMenu->setVisible( true );
	m_pBuyMenu->UpdateState(TYPE_FULL);
...
void TeamFortressViewport::CreateBuyMenu()
{
    m_pBuyMenu = new CBuyMenu();
    m_pBuyMenu->setParent( this );
    m_pBuyMenu->setVisible( false );
}

void TeamFortressViewport::CreateJoinMenu()
{
    m_pJoinMenu = new CClassMenu();
    m_pJoinMenu->setParent( this );
    m_pJoinMenu->setVisible( false );
}

CMenuPanel* TeamFortressViewport::ShowTMenu(int PC, int PK)
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;

	if (!m_pCurrentMenu) m_pJoinMenu->setVisible( true );
	m_pJoinMenu->teamOnly = true;
	m_pJoinMenu->m_iPlayerClass = PC;
	m_pJoinMenu->m_iPlayerKit = PK;
	m_pJoinMenu->UpdateState(CLASS_TEAM);
...
CMenuPanel* TeamFortressViewport::ShowJoinMenu(int PC, int PK)
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
...
CMenuPanel* TeamFortressViewport::ShowUnlockMenu(int PC, int PK)
{
	// Don't open menus in demo playback
	if ( gEngfuncs.pDemoAPI->IsPlayingback() )
		return NULL;
	...
	m_pJoinMenu->m_iPlayerClass = PC;
	m_pJoinMenu->m_iPlayerKit = PK;
	m_pJoinMenu->UpdateState(CLASS_UNLOCK);
...
void TeamFortressViewport::UpdateCursorState()
{
	...
	if ( m_pJoinMenu->isVisible() || m_pBuyMenu->isVisible() /*|| m_pSpectatorPanel->m_menuVisible*/ || m_pCurrentMenu || m_pTeamMenu->isVisible() || m_pServerBrowser->isVisible() || GetClientVoiceMgr()->IsInSquelchMode() )
	{
...
{
	if (m_pBuyMenu->isVisible())
		return m_pBuyMenu->ProcessKey(down, keynum);

	else if (m_pJoinMenu->isVisible())
		return m_pJoinMenu->ProcessKey(down, keynum);  
...
int TeamFortressViewport::MsgFunc_JoinMenu(const char *pszName, int iSize, void *pbuf )
{
	BEGIN_READ( pbuf, iSize );
...
int TeamFortressViewport::MsgFunc_TeamScore( const char *pszName, int iSize, void *pbuf )
{
	...
	//LLAPb: I didn't get this shit, so all i can do is new info array creation...
	...
	if (found) g_ExtraTeamInfo[j].score = READ_SHORT();
	else
		for (j = 1; j <= MAX_TEAMS; j++ )
		{
			if ( g_ExtraTeamInfo[j].name[0] == '\0' )
			{
				strncpy ( g_ExtraTeamInfo[j].name, TeamName, MAX_TEAM_NAME );
				g_ExtraTeamInfo[j].score = READ_SHORT();
				break;
			}
		}
...
void TeamFortressViewport::SetMoneyAmount ( int money )
{
	if (m_pBuyMenu == NULL) return;

	m_pBuyMenu->SetMoneyAmount(money);
	m_pJoinMenu->SetMoneyAmount(money);
}
