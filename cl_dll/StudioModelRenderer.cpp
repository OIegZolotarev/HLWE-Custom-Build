...
//animate w_powerups models
	if ( strstr( m_pCurrentEntity->model->name, "w_powerups" ))
	{
		float timemod;
...
*/
/*
extern int alt_index;//LLAPb: and read this shit from here
extern int alt_anim;
extern int alt_frame;
*/
...
int CStudioModelRenderer::StudioDrawModel( int flags )
{
	...
	if (flags & STUDIO_RENDER)
	{
		...
		if (m_pCurrentEntity == gEngfuncs.GetViewModel())
		{
			...
			if (alt_index)
			{
				model_t *pweaponmodel = IEngineStudio.Mod_ForName("models/v_item.mdl", 0);//IEngineStudio.GetModelByIndex( alt_index )

				m_pStudioHeader = (studiohdr_t *)IEngineStudio.Mod_Extradata (pweaponmodel);
				IEngineStudio.StudioSetHeader( m_pStudioHeader );

				//LLAPb: reflect model to left hand:
				(*m_protationmatrix)[0][1] *= -1;
				(*m_protationmatrix)[1][1] *= -1;
				(*m_protationmatrix)[2][1] *= -1;
				//LLAPb: reflection end

				StudioSetupAlternateBones( alt_anim, alt_frame );
...
*/
/*
void CStudioModelRenderer::StudioSetupAlternateBones( int seq, int frame )
{
	int					i;
	double				f;
	...
	if (seq >=  m_pStudioHeader->numseq) 
	{
		seq = 0;
	}

	pseqdesc = (mstudioseqdesc_t *)((byte *)m_pStudioHeader + m_pStudioHeader->seqindex) + seq;
